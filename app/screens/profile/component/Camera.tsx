...
const Camera: React.FC<ICameraProps> = ({
  ...
  const takePicture = async () => {
    if (camera) {
      ...
      console.log('newdtaa', data);
      let imagePath = [];
      imagePath.push({path: data.uri});
      from === '' ? setCanvasPhotos(imagePath) : setExperiencePhotos(imagePath);

      closeCamera();

      // // onChangeImage(data.uri);
      // navigation.navigate(SCREENS.FILTER, {images: data.uri});
      console.log('dataaaaaa', data.uri);
  ...
  const openLibrary = () => {
    ...
    ImagePicker.openPicker({
      ...
      setExperiencePhotos(images.path);
